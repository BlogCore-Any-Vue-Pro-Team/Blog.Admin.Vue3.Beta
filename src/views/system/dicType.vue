...
const ruleForm = {
    code: [
        { required: true, message: '字典code不能为空', trigger: 'change' },
    ],
    name: [
...
const HandleDel = (row) => {
    ...
    ElMessageBox.confirm('确定删除么?')
        .then(() => {
            dicTypeDelete({ id: row.Id }).then((res) => {
                HandleSearch()
        ...
        ElMessageBox.confirm('确定提交么?')
            .then(() => {
                if (formData.value.Id) {
                    //编辑
                    dicTypePut(formData.value).then((res) => {
                        HandleSearch()
                    ...
                    //新增
                    dicTypePost(formData.value).then((res) => {
                        HandleSearch()
                        dialogVisible.value = false
...
const HandleSearch = (page) => {
    ...
    dicTypeGet(filters.value).then(res => {
        tableData.value = res.data.response.data;
        tableTotal.value = res.data.response.dataCount;
    });
}
// 跳转dicData
const HandleDicData = (row) => {
    router.push({ path: "/system/dicData", query: { code: row.code } })
}
</script>
<template>
    <!-- 搜索 -->
    ...
    <el-table ref="refTable" :data="tableData" highlight-current-row @selection-change="HandleSelectChange"
        ...
        <el-table-column prop="code" label="字典code" width="250">
            <template #default="{ row }">
                <el-link type="primary" @click="HandleDicData(row)">{{ row.code }}</el-link>
            </template>
        </el-table-column>
        <el-table-column prop="name" label="字典名称" min-width="250"></el-table-column>
        <el-table-column prop="content" label="字典内容" width="250"></el-table-column>
        <el-table-column prop="description" label="字典描述" width="200"></el-table-column>
        <el-table-column prop="content2" label="字典内容2" width="250"></el-table-column>
        <el-table-column prop="content3" label="字典内容3" width="250"></el-table-column>
